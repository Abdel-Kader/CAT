...
module.exports = {
            ...
            if (model === 'Medecin') {
                models.Rendezvous.findAll({
                    ...
                    if (rdv)
                        ...
                        res.status(404).json({ error: "Vous n'avez aucucne demande de rendez-vous en cours !" })
                }).catch(err => {
                ...
                models.Rendezvous.findAll({
                    ...
                    if (rdv)
                        ...
                        res.status(404).json({ error: "Vous n'avez fait aucune demande de rendez-vous !" })
                }).catch(err => {
                    res.status(500).json({ error: err })
                })
            }
    ...
    getPatientRdv: function (req, res) {
        ...
        if (userId < 0)
            ...
            models.Rendezvous.findAll({
                where: { PatientId: id_patient, MedecinId: id_medecin, status: 1 },
                order: [["start", "DESC"]],
            }).then(rdv => {
                if (rdv)
                    res.status(200).json({ rdv })
