					...
					<div className="container-fluid">
							...
							<div className="row">
								<div className="col-sm-3">
									...
									>
										...
										<strong>
											{patient ? <span>{patient.length}</span> : <span>0</span>}
										</strong>
									</div>
								</div>
								<div className="col-sm-3">
						...
						<div className={"row"}>
							<div className="col-md-10">
								<div className="box">
									{currentConsultations && currentConsultations.length > 0 ? (
										<>
											<div className="box-head">
												<h2>
													{currentConsultations != null ? (
														<span>{currentConsultations.length}</span>
													) : null}{" "}
															...
															{currentConsultations != null
																? currentConsultations.map((res, index) => (
																		<tr key={index}>
																			...
																			<td>{res.motif}</td>
																			<td>
																				<i onClick={()=> handleDemarrer(res.Patient, res.id)}
																					className="icofont-video"
																					style={{
																						color: "green",
                                                                                        fontSize: 20,
                                                                                        cursor:'pointer'
									...
									) : <strong style={{color: 'green', height:50}}>Vous n'avez aucune téléconsultation aujourd'hui</strong>}
								</div>
							</div>
						</div>
					</div>
